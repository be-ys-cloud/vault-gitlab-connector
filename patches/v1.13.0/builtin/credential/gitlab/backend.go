...
func Backend() *backend {
	...
	b.Backend = &framework.Backend{
		...
		Paths: append([]*framework.Path{pathConfig(&b), pathLoginToken(&b), pathLoginUserPass(&b), pathOauthLogin(&b), pathLoginJob(&b)}, allPaths...),
...
func (b *backend) TokenClient(baseUrl string, token string) *gitlab.Client {
	if strings.HasPrefix(token, "OAuth-") {
		ret, _ := gitlab.NewOAuthClient(strings.TrimPrefix(token, "OAuth-"), gitlab.WithBaseURL(baseUrl))
		return ret
	}
	ret, _ := gitlab.NewClient(token, gitlab.WithBaseURL(baseUrl))
	return ret
}

func (b *backend) UserPassClient(baseURL, username, password string) (*gitlab.Client, error) {

	client, err := gitlab.NewBasicAuthClient(username, password, gitlab.WithBaseURL(baseURL))
	if err != nil {
		return nil, err
	}
	return client, nil
